const ChatbotWidget = {
    ...
    faqData: {
        ...
        "what are stylepoints": {
            answer: "StylePoints are our reward points for rentals and good behavior! 💎\n\n**For this demo:**\n• Points are tracked **off‑chain** in our backend (FastAPI)\n• 1 point ≈ $0.01 rental credit\n• You can **redeem** points at checkout to lower the price\n\n**Ways to earn:**\n• 50 points per rental\n• 100 points for 5‑star reviews\n• 200 points for on‑time returns\n• 500 points welcome bonus for new users\n\n**Roadmap (future):**\n• Option to mint on Flare as tokens\n• Peer‑to‑peer transfers & trading\n• On‑chain verifiable balances\n\nYour points are real in the demo: try asking *“What’s my StylePoints balance?”* and I’ll check it for you!",
        ...
        "what's my stylepoints balance": {
            answer: "I can look that up! 🔍\n\nClick the quick button below or just type **balance**. If you’re signed in, I’ll fetch it from our API.",
            followUp: ["How do I redeem StylePoints?", "What are StylePoints?"]
        },
                    ...
                    <div class="chat-messages" id="chat-messages">
                        <div class="welcome-message" id="welcome-message">
                            ...
                            <div class="quick-questions">
                                ...
                                <div class="quick-question" onclick="ChatbotWidget.askBalance()">
                                  <div class="quick-question-icon">🧮</div>
                                  <div class="quick-question-text">What's my StylePoints balance?</div>
                                </div>
    ...
    async askBalance() {
      // Announce action
      this.addMessage("Checking your StylePoints balance…", true);
      this.showTypingIndicator();

      const u = this.getCurrentUserLocal();
      const email = u?.email;

      // If api() helper exists (from api.js) and we have an email, call backend
      if (typeof window.api === 'function' && email) {
        try {
          const res = await window.api(`/rewards/balance/${encodeURIComponent(email)}`);
          this.hideTypingIndicator();
          const bal = Number(res?.balance || 0);
          const usd = (bal / 100).toFixed(2);
          this.addMessage(`Your current balance is **${bal.toLocaleString()} points** (≈ **$${usd}**).\n\nTip: you can redeem points at checkout!`);
          return;
        } catch (e) {
      ...
      this.hideTypingIndicator();
      const localBal = Number(u?.stylePoints || 0);
      const usd = (localBal / 100).toFixed(2);
      if (email) {
        this.addMessage(`(Offline) For **${email}**, I see **${localBal.toLocaleString()} points** in local storage (≈ **$${usd}**).\n\nIf this looks wrong, make sure you’re signed in and the API is reachable.`);
      } else {
        this.addMessage("I couldn't find a signed‑in user. Please sign in first, then ask me again!");
      }
    },
    ...
    findBestMatch(question) {
        ...
        if (normalizedQuestion.includes('balance') || normalizedQuestion.includes('points balance')) {
          // Trigger the API-based lookup and return a neutral key so we don't show a static FAQ
          setTimeout(() => this.askBalance(), 10);
          return 'default';
        }
        return 'default';
    },
